defmodule SimonWeb.MemberResetPasswordLive do
  ...
  def handle_event("reset_password", %{"member" => member_params}, socket) do
    case HumanResources.reset_member_password(socket.assigns.member, member_params) do
      {:ok, _} ->
        {:noreply,
         socket
  ...
  def handle_event("validate", %{"member" => member_params}, socket) do
    changeset = HumanResources.change_member_password(socket.assigns.member, member_params)
    {:noreply, assign_form(socket, Map.put(changeset, :action, :validate))}
  end

  defp assign_member_and_token(socket, %{"token" => token}) do
    if member = HumanResources.get_member_by_reset_password_token(token) do
