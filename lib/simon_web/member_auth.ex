defmodule SimonWeb.MemberAuth do
  ...
  @max_age 60 * 60 * 24 * 60
  @remember_me_cookie "_simon_web_member_remember_me"
  ...
  def log_out_member(conn) do
    ...
    member_token && HumanResources.delete_member_session_token(member_token)

    if live_socket_id = get_session(conn, :live_socket_id) do
      SimonWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def fetch_current_member(conn, _opts) do
    ...
    member = member_token && HumanResources.get_member_by_session_token(member_token)
  ...
  the current_member:
      ...
      defmodule SimonWeb.PageLive do
        use SimonWeb, :live_view

        on_mount {SimonWeb.MemberAuth, :mount_current_member}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{SimonWeb.MemberAuth, :ensure_authenticated}] do
  ...
  defp mount_current_member(socket, session) do
    Phoenix.Component.assign_new(socket, :current_member, fn ->
      if member_token = session["member_token"] do
        HumanResources.get_member_by_session_token(member_token)
      end
    end)
  end
